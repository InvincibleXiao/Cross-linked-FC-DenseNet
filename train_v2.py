...
if __name__ == '__main__':
    ...
    for epoch in range (num_epoch):
        ...
        # Save checkpoint
        if (epoch % 1000) == 0:# or epoch == (num_epoch - 1):# or (epoch % 1000) == 0:
            torch.save(model_S.state_dict(), './checkpoints/' + '%s_%s.pth' % (str(epoch).zfill(5), checkpoint_name))

end = time.time()
print('The total time for training : {:.1f} minutes = {:.1f} hours'.format((end-start)/60,
